...
- [x] Record a completed macOS and Windows-Git-Bash run for the exact PR head.

Acceptance: the supported Git Bash path is verified rather than inferred from
the default Windows shell.

Evidence: GitHub Actions [verify run 30482429529](https://github.com/mortenbroesby/agent-distro/actions/runs/30482429529)
passed `npm ci`, `npm test`, and `npm run test:proof` on macOS and Windows Git
Bash for PR #16's exact head.
...
- [x] Reconcile `.memory/00` and `.memory/01` with the implemented contract.
- [x] Check acceptance criteria only with direct tests or completed hosted evidence.
- [x] Move this epic to `../done/` when every story is complete.
