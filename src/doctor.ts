...
export function verify(target: string) {
  ...
  try {
    ...
    if (manifest.tool !== "agent-distro" || manifest.version !== 1 || typeof manifest.catalogVersion !== "string" || !Array.isArray(manifest.files)) throw new Error("invalid manifest");
  ...
  if (snapshot.target.exists) {
    ...
    if (snapshot.target.directory) {
      ...
      if (snapshot.manifest.present) try {
        ...
        snapshot.manifest.valid = manifest.tool === "agent-distro" && manifest.version === 1 && typeof manifest.catalogVersion === "string" && Array.isArray(manifest.files);
