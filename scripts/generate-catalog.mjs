...
const pluginFiles = new Map([
  ["plugin.json", JSON.stringify({
    name: "agent-distro",
    description: "Curated Copilot agents, skills, and optional MCP and hook templates for evidence-driven engineering workflows.",
    version: "0.1.0",
...
const marketplace = JSON.stringify({
  ...
  plugins: [{
    ...
    description: "Curated agents, skills, and optional MCP and hook templates for evidence-driven engineering workflows.",
    version: "0.1.0",
    strict: true,
  }],
}, null, 2) + "\n";
...
for (const [relative, content] of pluginFiles) {
  const output = relative === ".github/plugin/marketplace.json" ? path.join(root, relative) : path.join(plugin, relative);
  if (check) {
    if (!fs.existsSync(output) || fs.readFileSync(output, "utf8") !== content) throw new Error("generated Copilot plugin is stale: " + relative);
  } else {
    fs.mkdirSync(path.dirname(output), { recursive: true });
    fs.writeFileSync(output, content);
  }
